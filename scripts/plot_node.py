...
class ReferenceStateNode(Node):
    ...
    def __init__(self, robot_no):
        ...
        self.v1_curr_pose = (0.0, 0.0)
        self.v2_curr_pose = (0.0, 0.0)
        ...
        # Create a subscriber
        self.v1_subscription = self.create_subscription(
            PoseArray,
            f'/v_1/pose',
            self.v1_pose_callback,
            10  # QoS profile depth
        )
        
        self.v2_subscription = self.create_subscription(
            PoseArray,
            f'/v_2/pose',
            self.v2_pose_callback,
            10  # QoS profile depth
        )
        
        self.plot_publisher_ = self.create_publisher(Float32MultiArray, '/plot_node', 10)
    ...
    def v1_pose_callback(self, msg):
        ...
        with self.lock:
            if msg.poses:
                ...
                self.v1_curr_pose = (first_pose.position.x, first_pose.position.y)
                self.curr_pose = (first_pose.position.x, first_pose.position.y)

                quaternion = first_pose.orientation
                self.v1_theta = self.quaternion_to_theta(quaternion)
    ...
    def v2_pose_callback(self, msg):
        ...
        with self.lock:
            if msg.poses:
                ...
                first_pose = msg.poses[0]
                self.v2_curr_pose = (first_pose.position.x, first_pose.position.y)

                quaternion = first_pose.orientation
                self.v2_theta = self.quaternion_to_theta(quaternion)
    ...
    def getRefTheta(self):
        self.theta_r = math.atan((2 * (self.a_y)[2] * self.T + (self.a_y)[1]) / (2 * (self.a_x)[2] * self.T + (self.a_x)[1]))
    ...
    def publish_plot(self):
        ...
        dist_diff = math.sqrt(((self.v1_curr_pose)[0] - (self.v2_curr_pose)[0]) ** 2 + ((self.v1_curr_pose)[1] - (self.v2_curr_pose)[1]) ** 2) 
        msg.data = [dist_diff]
